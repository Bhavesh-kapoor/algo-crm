...
if (Auth::user()->role == 'Manager') {
    $users = User::join('workunder', 'workunder.user_id', '=', 'users.id')
                ->where('workunder.manager_id', Auth::user()->id)
                ->select('users.*')
                ->get();
...
{
    $insert = [
        'total_amount' => $data['totalSettlement'],
        'lead_id' => $data['lead_id'],
        'phase_name' => $data['settlementfor'],
        'add_by' => Auth::user()->id,
        'created_at' => now(),
    ];

    // Set your manual public_html path here
    $publicHtmlPath = '/home/u239133126/domains/wayone.trade/public_html/';

    // Upload Quotation
    if ($data->hasFile('quotation')) {
        $file = $data->file('quotation');
        if ($file->isValid()) {
            $imageName = time() . '_quotation.pdf';
            $destination = $publicHtmlPath . 'uploads/quotations/';
    ...
    if ($data->hasFile('recepit')) {
        ...
        if ($file->isValid()) {
            ...
            $imageName = time() . '_receipt.' . $extension;
            $destination = $publicHtmlPath . 'uploads/receipts/';
            if (!file_exists($destination)) {
                mkdir($destination, 0755, true);
            }
            $file->move($destination, $imageName);
    ...
    if ($phaseAdded) {
        return response()->json(['code' => 200, 'message' => 'Quotation uploaded successfully!']);
    } else {
